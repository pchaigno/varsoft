\chapter{Contexte mathématique}
	...
	\section{Particularités des séries financières}
	...
	\section{Modèle GARCH}
		...
		\subsection{Lien avec la Value at Risk}
			...
			Cela exprime bien le fait que la probabilité que la valeur de la perte ($V_t - V_{t+h}$) dépasse la VaR vaut $\alpha$. En partant de cette définition, on arrive à montrer que $VaR_{t+h}(\alpha) = 1 - exp{(q_{t}(h,\alpha))}*V_{t}$, avec $q_{t}(h,\alpha)$ le quantile d'ordre $\alpha$ de $r_{t+h}+r_{t+h-1}+r_{t+h-2}+...+r_{t+1}$. On voit donc ici que la VaR est caluclée à partir de rendements futurs et donc encore non observés. Il est donc nécessaire d'estimer ces rendements, ce qui rend le calcul de la VaR difficile.
